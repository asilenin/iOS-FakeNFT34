...
actor MockCatalogService: CatalogServiceProtocol {
    ...
    init() {
        self.collections = MockCatalogService.makeMockCollections()
    }
...
private extension MockCatalogService {

    static func makeMockCollections() -> [NftCollection] {
        [
            NftCollection(
                id: "1",
                name: "Peach",
    ...
    static let mockAuthorWebsite: URL = makeURL("https://practicum.yandex.ru")

    /// Path "Обложки_коллекций" is percent-encoded for compatibility with `URL(string:)`.
    static func coverURL(for name: String) -> URL {
